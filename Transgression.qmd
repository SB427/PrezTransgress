...
-   Utilisation de la méthode de classification descendante hiérarchique (CHD) de Reinert (comme dans Iramuteq) grâce que package [rainette](https://juba.github.io/rainette/index.html)
...
-   `r scales::number(nrow(author), big.mark = " ")` auteurs, en moyenne `r str_split(data$author,";")%>%lengths()%>%mean()%>%round(digits = 2)` auteurs par articles

-   Le corpus s'étend de `r min(data$year)` à `r max(data$year)`

-   Longueur moyenne d'un article : `r scales::number(mean(data$ntoken), big.mark = " ", accuracy = 1)` (min : `r scales::number(min(data$ntoken), big.mark = " ", accuracy = 1)` ; max : `r scales::number(max(data$ntoken), big.mark = " ", accuracy = 1)`)
